...
time = np.arange(600)
print(all_num)
result_data = np.ones((all_num,17,600))* 1e10
result_time = np.ones((all_num,17,600))* -1e10
encoder_mask = np.zeros((all_num,17))
# static_data = np.zeros((all_num,9))

for i in tqdm(range(all_num)):
    curr_data = data[i]
    curr_series = curr_data
    curr_time = time
    sensor_time = time
